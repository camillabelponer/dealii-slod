...
main(int argc, char **argv)
{
  ...
  // create matrix
  // TrilinosWrappers::
  SparsityPattern        sparsity_pattern; //(n_dofs, n_dofs);
  DynamicSparsityPattern dsp(dof_handler.n_dofs());
  DoFTools::make_sparsity_pattern(dof_handler, dsp);
  sparsity_pattern.copy_from(dsp);
  // DoFTools::make_sparsity_pattern(dof_handler, sparsity_pattern,
  // constraints); sparsity_pattern.compress();

  // TrilinosWrappers::SparseMatrix
  SparseMatrix<double> sparse_matrix_0(sparsity_pattern);

  MatrixCreator::create_laplace_matrix<dim, dim>(
    mapping, dof_handler, quadrature, sparse_matrix_0, nullptr, constraints);

  TrilinosWrappers::SparseMatrix sparse_matrix;
  sparse_matrix.reinit(sparse_matrix_0);
  ...
  for (unsigned int b = 0; b < Ndofs_coarse; ++b)
    {
      ...
      // SolverCG<Vector<double>> sd(sc);
      sd.solve(sparse_matrix, u_i, P_e_i); //, PreconditionIdentity());
